...
class Sharer:
    ...
    def doShare(self,tableName,updateTime):
        if tableName=='news':
            ...
            sql='''INSERT INTO rec_news(id,title,update_time,type,agency,head1,head2,key_data,label_data) SELECT news_id,news_title,update_time,news_type,agency_name,head1,head2,keys_data,news_label FROM tmp_news WHERE update_time=%s '''%(updateTime)
        else:
